...
class ClickHouseHistoricalStoreIT {
  ...
  @Test
  void persistsFieldsAndCollapsesReplayedSignalId() throws Exception {
    ...
    try (var connection =
        ...
        var rows =
            statement.executeQuery(
                "SELECT signal_id,actor_id,risk_score FROM risk_signals FINAL")) {
      assertThat(rows.next()).isTrue();
      assertThat(rows.getString(1)).isEqualTo("same-id");
      assertThat(rows.getString(2)).isEqualTo("actor");
      assertThat(rows.getInt(3)).isEqualTo(80);
