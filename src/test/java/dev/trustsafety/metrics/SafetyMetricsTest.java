...
class SafetyMetricsTest {
  @Test
  void registersAndUpdatesEveryMetricIncludingQuantiles() {
    MetricListener listener = new MetricListener();
    SafetyMetrics metrics = new SafetyMetrics(listener.getMetricGroup());
    metrics.onEvent(event(900, 800), 1_000);
  ...
  private static SafetyEvent event(long occurred, long ingested) {
    return new SafetyEvent(
        1,
        "e" + occurred,
        Instant.ofEpochMilli(occurred),
        Instant.ofEpochMilli(ingested),
        "a",
        null,
