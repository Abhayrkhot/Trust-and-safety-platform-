...
public final class RedisHotStateStore implements RiskSignalStore {
  ...
  @Override
  public void write(RiskSignal signal) throws IOException {
    ...
    value.put("observed_event_count", signal.observedEventCount());
    value.put("observed_severity_sum", signal.observedSeveritySum());
    ...
    connection
        ...
        .eval(
            ...
            new String[] {key(signal.actorId())},
            Long.toString(signal.emittedAt().toEpochMilli()),
            JSON.writeValueAsString(value),
            Long.toString(ttlMillis));
  }

  public static String key(String actorId) {
    return "safety:risk:actor:" + actorId;
  }
