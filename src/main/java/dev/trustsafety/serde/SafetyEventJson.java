...
public final class SafetyEventJson {
  ...
  public static SafetyEvent decode(byte[] payload) throws IOException {
    ...
    try {
      ...
      return new SafetyEvent(
          version,
          eventId,
          Instant.parse(requiredText(n, "occurred_at")),
          Instant.parse(requiredText(n, "ingested_at")),
          version == 1 ? "default" : requiredText(n, "tenant_id"),
          version == 1 ? eventId : optionalText(n, "trace_id"),
          requiredText(n, "actor_id"),
          optionalText(n, "content_id"),
          SafetyEvent.EventType.valueOf(requiredText(n, "event_type")),
  ...
  private static String requiredText(JsonNode n, String f) {
    JsonNode v = n.get(f);
    if (v == null || !v.isTextual() || v.textValue().isBlank())
      throw new IllegalArgumentException(f + " must be text");
    return v.textValue();
  }

  private static String optionalText(JsonNode n, String f) {
    JsonNode v = n.get(f);
    return v == null || v.isNull() ? null : requiredText(n, f);
  }

  private static int requiredInt(JsonNode n, String f) {
    JsonNode v = n.get(f);
    if (v == null || !v.canConvertToInt())
      throw new IllegalArgumentException(f + " must be integer");
    return v.intValue();
  }

  private static Map<String, String> attributes(JsonNode n) {
