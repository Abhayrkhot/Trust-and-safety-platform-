...
public final class SafetyProcessor extends KeyedProcessFunction<String, SafetyEvent, RiskSignal> {
  ...
  public SafetyProcessor(List<RuleConfig> rules, long dedupTtlMillis) {
    ...
    this.rules=List.copyOf(rules); this.dedupTtlMillis=dedupTtlMillis;
  ...
  @Override public void open(Configuration ignored) {
    MapStateDescriptor<String,Boolean> descriptor=new MapStateDescriptor<>("seen-event-ids",String.class,Boolean.class);
    descriptor.enableTimeToLive(StateTtlConfig.newBuilder(Time.milliseconds(dedupTtlMillis)).setUpdateType(StateTtlConfig.UpdateType.OnCreateAndWrite).setStateVisibility(StateTtlConfig.StateVisibility.NeverReturnExpired).build());
