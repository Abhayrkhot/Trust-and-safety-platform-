...
public record RuleConfig(
    ...
    long windowMillis,
    long minimumEvents,
    long minimumSeveritySum,
    int riskScore,
    Set<SafetyEvent.EventType> eventTypes,
    Map<String, String> requiredAttributes)
    implements Serializable {
  public RuleConfig {
    if (ruleId == null || ruleId.isBlank())
      throw new IllegalArgumentException("ruleId must not be blank");
    if (windowMillis <= 0 || minimumEvents <= 0 || minimumSeveritySum < 0)
  ...
  public RuleConfig(
      ...
      long minimumSeveritySum,
      int riskScore) {
    this(ruleId, windowMillis, minimumEvents, minimumSeveritySum, riskScore, Set.of(), Map.of());
  }

  public boolean matches(SafetyEvent event) {
    return (eventTypes.isEmpty() || eventTypes.contains(event.eventType()))
        && requiredAttributes.entrySet().stream()
            .allMatch(e -> e.getValue().equals(event.attributes().get(e.getKey())));
  }
}
